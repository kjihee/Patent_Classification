...
class network2vec(object):
    def __init__(self, labels, filename, class_name, model, embed_dir=None,
            previous_model=None, is_network=True):
        self._labels = labels
        self._model = model
    ...
    def preprocessing(self, filename):
        ...
        df = pd.read_csv(filename, sep='\s+', header = None, names=["idx","cited"])
        df["values"] = 1
    ...
    def train_word2vec(self):
        ...
        p2v = Word2Vec(alpha=0.025, window=8, min_count=1, min_alpha=0.020, size=300)
    ...
    def train_doc2vec(self):
        ...
        p2v = Doc2Vec(alpha=0.025, window=9, min_count=1, min_alpha=0.020, size=300)
    ...
    def save_embed_file(self, save_path, X, labels):
        with open(save_path,'w') as f:
            ...
            for idx, label in enumerate(labels):
                f.write(str(label) +" "+" ".join([str(x) for x in X[idx]]))
            f.close()
